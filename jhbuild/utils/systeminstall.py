...
class SystemInstall(object):
    ...
    def install(self, uninstalled, sysdeps_assume_yes):
...
class AptSystemInstall(SystemInstall):
    ...
    def _install_packages(self, native_packages, sysdeps_assume_yes):
        logging.info(_('Installing: %(pkgs)s') % {'pkgs': ' '.join(native_packages)})
        apt_cmd_line = ['apt-get']
        if sysdeps_assume_yes is True:
    ...
    def install(self, uninstalled, sysdeps_assume_yes):
        ...
        if native_packages:
            self._install_packages(native_packages, sysdeps_assume_yes=sysdeps_assume_yes)
        else:
            logging.info(_('Nothing to install'))
